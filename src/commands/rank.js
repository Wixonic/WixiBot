...
const info = {
	...
	run: async (logger, bot, interaction) => {
		...
		await interaction.followUp({
			allowedMentions: {},
			content: userRank.description + `\n[See more](<${new URL(`/discord/user/?id=${targetUser.id}`, bot.settings.secrets.server.url)}>)` + (targetUser.id == interaction.user.id && !userRank.linked ? `\n### Get 5,000 points for free by [linking your account](<${new URL(`/discord/user/@me`, bot.settings.secrets.server.url)}>)!` : "")
		});
	}
};
