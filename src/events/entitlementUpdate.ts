import { Events, type Entitlement, type Guild } from "discord.js";
			...
			for (const discordGuild of guilds) {
				...
				if (member) {
					try {
						...
						logger.error(`Failed to remove supporter role from ${entitlement.userId} in guild ${discordGuild.id}`, {
							cause: error
						});
					}
				}
