...
export const event = {
	...
	async execute(logger: Logger, entitlement: Entitlement) {
		...
		if (fundingSku && entitlement.skuId === fundingSku) {
			if (entitlement.guildId) {
				...
				if (discordGuild) {
					...
					if (member) {
						...
						return;
					} else logger.warn(`Skipped supporter role update for ${entitlement.userId}: entitlement guild ${entitlement.guildId} not found (sku: ${entitlement.skuId}).`);
				}
			...
			} else logger.warn(`Skipped supporter role update for ${entitlement.userId}: missing entitlement guild ID (sku: ${entitlement.skuId}).`);
		}
	}
};