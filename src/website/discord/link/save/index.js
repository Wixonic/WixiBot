...
addEventListener("DOMContentLoaded", async () => {
	...
	if (params.has("id") && params.has("username") && user.uid == params.get("uid")) {
		await firebase.link("discord", {
			id: params.get("id"),
			username: params.get("username")
		});

		location.href = params.has("redirect") ? decodeURIComponent(params.get("redirect")) : (localEnvironment ? path.local.accounts : path.accounts);
	} else location.href = new URL(`/discord/link/?redirect=${encodeURIComponent(location.href)}`, localEnvironment ? path.local.server : path.server);
});