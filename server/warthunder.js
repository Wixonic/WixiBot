	...
	try {
		...
		if (errors.length == 0) {
			...
			for (const obj of objs) {
				if (["ground_model", "aircraft"].includes(obj.type)) svgPoints.push(`<circle cx="${obj.x * size[0]}" cy="${obj.y * size[1]}" r="${Math.max(...size) / 100}" fill="${obj.color}" stroke="#FFF" stroke-width="${Math.max(...size) / 1000}" />`);
				if (["capture_zone"].includes(obj.type)) svgPoints.push(`<circle cx="${obj.x * size[0]}" cy="${obj.y * size[1]}" r="${Math.max(...size) / 50}" fill="${obj.color}" stroke="#FFF" stroke-width="${Math.max(...size) / 1000}" />`);
			}
