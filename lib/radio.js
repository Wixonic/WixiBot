...
const Radio = {
	 ...
	 */
	async load(song) {
		song.path = path.join(config.cache.radio, randomUUID() + ".mp3");

		try {
			await new Promise((resolve, reject) => {
				const command = `yt-dlp -x --audio-format mp3 -o "${song.path}" https://www.youtube.com/watch?v=${song.youtubeId}`;
